...
foxglove::WebSocketServer startServer(foxglove::WebSocketServerOptions&& options) {
  ...
  auto server = std::move(result.value());  // NOLINT(bugprone-unchecked-optional-access)
    ...
    auto wait_result = cv.wait_for(lock, kTestTimeout, [&] {
      if (server_set_parameters.has_value()) {
        ...
        if (params[2].isByteArray()) {
          ...
          REQUIRE(result.has_value());
          auto bytes = result.value();  // NOLINT(bugprone-unchecked-optional-access)
...
TEST_CASE("Log message to websocket sinks") {
  ...
  auto server = startServer(context, foxglove::WebSocketServerCapabilities(0), std::move(cb));  // NOLINT(clang-analyzer-optin.core.EnumCastOutOfRange)
